#pragma warning(disable : 4996) // Disables legacy C4996 warnings for standard file I/O
#include "cutscene.h"
#include <stdio.h>
#include <string.h>
...
void LoadCutscene(Cutscene* cs, const char* filename)
{
    ...
    // Clear any old text data from memory (Zeroing the buffer)
    ...
    while (fgets(buffer, sizeof(buffer), f))
    {
        ...
        // Ignore curly braces (Notes in the script for writers)
        ...
        if (len == 0) {
            if (strlen(cs->dialogs[currentIdx]) > 0) {
                currentIdx++;
                if (currentIdx >= MAX_DIALOGS) break; // Array protection
            }
        }
        else {
            ...
            // Buffer boundary protection to prevent array overflows
            if (currentLen + addedLen + 2 < MAX_CHAR) {
                ...
                // If the box is full, forcibly move to a new one
                currentIdx++;
    ...
    fclose(f); // Close the file stream to prevent leaks
...
bool UpdateCutscene(Cutscene* cs)
{
    ...
    {
        // If text is still typing, show all of it immediately (Skip typing)
...
void DrawCutscene(Cutscene* cs)
{
    ...
    {
        // Math to calculate Y position: the current box is at 500, older ones are pushed higher up
        int yPos = 500 - ((cs->currentDialog - i) * 80);

        if (yPos < -50) continue; // Offscreen top, don't draw to save resources
        ...
        if (i == cs->currentDialog) {
            ...
            if (charsToCopy >= MAX_CHAR) charsToCopy = MAX_CHAR - 1; // Safeguard

            strncpy(temp, cs->dialogs[i], charsToCopy);
            temp[charsToCopy] = '\0'; // Null terminate the string

            DrawText(temp, 80, yPos, 20, textColor);
        }
        else {
            // Lines from history are drawn darker to focus attention on the active line
    ...
    int maxChars = (int)strlen(cs->dialogs[cs->currentDialog]);
    if (cs->charCount >= maxChars) {
        if ((int)(GetTime() * 2) % 2 == 0) { // Math to blink precisely every 0.5s
